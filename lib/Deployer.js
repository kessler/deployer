...
function Deployer() {
	...
	this._updateTasks = [
		...
		_.bind(fs.mkdir, fs, conf.prodPath),
		_.partial(ncp, conf.dirtyPath, conf.prodPath),
		_.bind(childProcess.exec, childProcess, conf.startCommand, { cwd: conf.prodPath })
	];
}
...
p.start = function() {
	...
	sock.bind(conf.port); // TODO: unref it
	sock.server.unref();
	this._sock = sock;
	this._pull();
};
...
p._onDepsUpdated = function(err) {
	...
	if (err) {
		...
		setTimeout(this._updateDepsBound, 60000);
		return;
	}
	this._update();
};
...
p._stopService = function(callback) {
	...
	logger.debug('Stop using: %s', conf.stopCommand);
